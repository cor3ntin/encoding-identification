...
constexpr inline enc_data data[] = {{3, "US-ASCII"},
                                    {3, "ANSI_X3.4-1968"},
                                    {3, "ANSI_X3.4-1986"},
                                    {3, "IBM367"},
                                    {3, "ISO646-US"},
...
constexpr bool encoding_is(const char* name) {
    ...
    if constexpr(id_ == details::id::ASCII) {
        return do_compare({"US-ASCII", "ANSI_X3.4-1968", "ANSI_X3.4-1986", "IBM367", "ISO646-US",
                           "ISO_646.irv:1991", "cp367", "csASCII", "iso-ir-6", "us"},
                          name);
    }

    if constexpr(id_ == details::id::ISOLatin1) {
        return do_compare({"ISO-8859-1", "ISO_8859-1:1987", "CP819", "IBM819", "ISO_8859-1",
                           "csISOLatin1", "iso-ir-100", "l1", "latin1"},
                          name);
    }

    if constexpr(id_ == details::id::ISOLatin2) {
        return do_compare({"ISO-8859-2", "ISO_8859-2:1987", "ISO_8859-2", "csISOLatin2",
                           "iso-ir-101", "l2", "latin2"},
                          name);
    }

    if constexpr(id_ == details::id::ISOLatin3) {
        return do_compare({"ISO-8859-3", "ISO_8859-3:1988", "ISO_8859-3", "csISOLatin3",
                           "iso-ir-109", "l3", "latin3"},
                          name);
    }

    if constexpr(id_ == details::id::ISOLatin4) {
        return do_compare({"ISO-8859-4", "ISO_8859-4:1988", "ISO_8859-4", "csISOLatin4",
                           "iso-ir-110", "l4", "latin4"},
                          name);
    }

    if constexpr(id_ == details::id::ISOLatinCyrillic) {
        return do_compare({"ISO-8859-5", "ISO_8859-5:1988", "ISO_8859-5", "csISOLatinCyrillic",
                           "cyrillic", "iso-ir-144"},
                          name);
    }

    if constexpr(id_ == details::id::ISOLatinArabic) {
        return do_compare({"ISO-8859-6", "ISO_8859-6:1987", "ASMO-708", "ECMA-114", "ISO_8859-6",
                           "arabic", "csISOLatinArabic", "iso-ir-127"},
                          name);
    }

    if constexpr(id_ == details::id::ISOLatinGreek) {
        return do_compare({"ISO-8859-7", "ISO_8859-7:1987", "ECMA-118", "ELOT_928", "ISO_8859-7",
                           "csISOLatinGreek", "greek", "greek8", "iso-ir-126"},
                          name);
    }

    if constexpr(id_ == details::id::ISOLatinHebrew) {
        return do_compare({"ISO-8859-8", "ISO_8859-8:1988", "ISO_8859-8", "csISOLatinHebrew",
                           "hebrew", "iso-ir-138"},
                          name);
    }

    if constexpr(id_ == details::id::ISOLatin5) {
        return do_compare({"ISO-8859-9", "ISO_8859-9:1989", "ISO_8859-9", "csISOLatin5",
    ...
    if constexpr(id_ == details::id::ISOLatin6) {
        return do_compare(
            {"ISO-8859-10", "ISO_8859-10:1992", "csISOLatin6", "iso-ir-157", "l6", "latin6"}, name);
    }
    ...
    if constexpr(id_ == details::id::EUCFixWidJapanese) {
        return do_compare({"Extended_UNIX_Code_Fixed_Width_for_Japanese", "csEUCFixWidJapanese"},
                          name);
    }

    if constexpr(id_ == details::id::ISO4UnitedKingdom) {
        return do_compare({"BS_4730", "ISO646-GB", "csISO4UnitedKingdom", "gb", "iso-ir-4", "uk"},
                          name);
    }

    if constexpr(id_ == details::id::ISO11SwedishForNames) {
        return do_compare(
            {"SEN_850200_C", "ISO646-SE2", "csISO11SwedishForNames", "iso-ir-11", "se2"}, name);
    }
    ...
    if constexpr(id_ == details::id::ISO60DanishNorwegian) {
        return do_compare({"NS_4551-1", "ISO646-NO", "csISO60DanishNorwegian", "csISO60Norwegian1",
                           "iso-ir-60", "no"},
                          name);
    }
    ...
    if constexpr(id_ == details::id::ISO10Swedish) {
        return do_compare(
            {"SEN_850200_B", "FI", "ISO646-FI", "ISO646-SE", "csISO10Swedish", "iso-ir-10", "se"},
            name);
    }

    if constexpr(id_ == details::id::KSC56011987) {
        return do_compare({"KS_C_5601-1987", "KSC_5601", "KS_C_5601-1989", "csKSC56011987",
                           "iso-ir-149", "korean"},
                          name);
    }
    ...
    if constexpr(id_ == details::id::ISO13JISC6220jp) {
        return do_compare({"JIS_C6220-1969-jp", "JIS_C6220-1969", "csISO13JISC6220jp", "iso-ir-13",
                           "katakana", "x0201-7"},
                          name);
    }
    ...
    if constexpr(id_ == details::id::ISO14JISC6220ro) {
        return do_compare(
            {"JIS_C6220-1969-ro", "ISO646-JP", "csISO14JISC6220ro", "iso-ir-14", "jp"}, name);
    }
    ...
    if constexpr(id_ == details::id::ISO54271981) {
        return do_compare({"ISO_5427:1981", "ISO5427Cyrillic1981", "csISO54271981", "iso-ir-54"},
                          name);
    }
    ...
    if constexpr(id_ == details::id::ISO61Norwegian2) {
        return do_compare({"NS_4551-2", "ISO646-NO2", "csISO61Norwegian2", "iso-ir-61", "no2"},
                          name);
    }
    ...
    if constexpr(id_ == details::id::ISO87JISX0208) {
        return do_compare(
            {"JIS_C6226-1983", "JIS_X0208-1983", "csISO87JISX0208", "iso-ir-87", "x0208"}, name);
    }
    ...
    if constexpr(id_ == details::id::ISO91JISC62291984a) {
        return do_compare({"JIS_C6229-1984-a", "csISO91JISC62291984a", "iso-ir-91", "jp-ocr-a"},
                          name);
    }

    if constexpr(id_ == details::id::ISO92JISC62991984b) {
        return do_compare({"JIS_C6229-1984-b", "ISO646-JP-OCR-B", "csISO92JISC62991984b",
                           "iso-ir-92", "jp-ocr-b"},
                          name);
    }
    ...
    if constexpr(id_ == details::id::ISO93JIS62291984badd) {
        return do_compare(
            {"JIS_C6229-1984-b-add", "csISO93JIS62291984badd", "iso-ir-93", "jp-ocr-b-add"}, name);
    }

    if constexpr(id_ == details::id::ISO94JIS62291984hand) {
        return do_compare(
            {"JIS_C6229-1984-hand", "csISO94JIS62291984hand", "iso-ir-94", "jp-ocr-hand"}, name);
    }

    if constexpr(id_ == details::id::ISO95JIS62291984handadd) {
        return do_compare({"JIS_C6229-1984-hand-add", "csISO95JIS62291984handadd", "iso-ir-95",
                           "jp-ocr-hand-add"},
                          name);
    }
    ...
    if constexpr(id_ == details::id::ISO99NAPLPS) {
        return do_compare(
            {"ANSI_X3.110-1983", "CSA_T500-1983", "NAPLPS", "csISO99NAPLPS", "iso-ir-99"}, name);
    }
    ...
    if constexpr(id_ == details::id::ISO121Canadian1) {
        return do_compare({"CSA_Z243.4-1985-1", "ISO646-CA", "ca", "csISO121Canadian1", "csa7-1",
                           "csa71", "iso-ir-121"},
                          name);
    }

    if constexpr(id_ == details::id::ISO122Canadian2) {
        return do_compare({"CSA_Z243.4-1985-2", "ISO646-CA2", "csISO122Canadian2", "csa7-2",
                           "csa72", "iso-ir-122"},
                          name);
    }
    ...
    if constexpr(id_ == details::id::ISO141JUSIB1002) {
        return do_compare(
            {"JUS_I.B1.002", "ISO646-YU", "csISO141JUSIB1002", "iso-ir-141", "js", "yu"}, name);
    }
    ...
    if constexpr(id_ == details::id::ISO147Macedonian) {
        return do_compare({"JUS_I.B1.003-mac", "csISO147Macedonian", "iso-ir-147", "macedonian"},
                          name);
    }
    ...
    if constexpr(id_ == details::id::ISO151Cuba) {
        return do_compare({"NC_NC00-10:81", "ISO646-CU", "csISO151Cuba", "cuba", "iso-ir-151"},
                          name);
    }
    ...
    if constexpr(id_ == details::id::ISO153GOST1976874) {
        return do_compare({"GOST_19768-74", "ST_SEV_358-88", "csISO153GOST1976874", "iso-ir-153"},
                          name);
    }
    ...
    if constexpr(id_ == details::id::ISO885914) {
        return do_compare({"ISO-8859-14", "ISO_8859-14", "ISO_8859-14:1998", "csISO885914",
                           "iso-celtic", "iso-ir-199", "l8", "latin8"},
                          name);
    }
    ...
    if constexpr(id_ == details::id::ISO885916) {
        return do_compare({"ISO-8859-16", "ISO_8859-16", "ISO_8859-16:2001", "csISO885916",
                           "iso-ir-226", "l10", "latin10"},
                          name);
    }
    ...
    if constexpr(id_ == details::id::IBM037) {
        return do_compare({"IBM037", "cp037", "csIBM037", "ebcdic-cp-ca", "ebcdic-cp-nl",
                           "ebcdic-cp-us", "ebcdic-cp-wt"},
                          name);
    }
    ...
    if constexpr(id_ == details::id::IBM00858) {
        return do_compare(
            {"IBM00858", "CCSID00858", "CP00858", "PC-Multilingual-850+euro", "csIBM00858"}, name);
    }

    if constexpr(id_ == details::id::IBM00924) {
        return do_compare(
            {"IBM00924", "CCSID00924", "CP00924", "csIBM00924", "ebcdic-Latin9--euro"}, name);
    }

    if constexpr(id_ == details::id::IBM01140) {
        return do_compare({"IBM01140", "CCSID01140", "CP01140", "csIBM01140", "ebcdic-us-37+euro"},
                          name);
    }

    if constexpr(id_ == details::id::IBM01141) {
        return do_compare({"IBM01141", "CCSID01141", "CP01141", "csIBM01141", "ebcdic-de-273+euro"},
                          name);
    }

    if constexpr(id_ == details::id::IBM01142) {
        return do_compare({"IBM01142", "CCSID01142", "CP01142", "csIBM01142", "ebcdic-dk-277+euro",
                           "ebcdic-no-277+euro"},
                          name);
    }

    if constexpr(id_ == details::id::IBM01143) {
        return do_compare({"IBM01143", "CCSID01143", "CP01143", "csIBM01143", "ebcdic-fi-278+euro",
                           "ebcdic-se-278+euro"},
                          name);
    }

    if constexpr(id_ == details::id::IBM01144) {
        return do_compare({"IBM01144", "CCSID01144", "CP01144", "csIBM01144", "ebcdic-it-280+euro"},
                          name);
    }

    if constexpr(id_ == details::id::IBM01145) {
        return do_compare({"IBM01145", "CCSID01145", "CP01145", "csIBM01145", "ebcdic-es-284+euro"},
                          name);
    }

    if constexpr(id_ == details::id::IBM01146) {
        return do_compare({"IBM01146", "CCSID01146", "CP01146", "csIBM01146", "ebcdic-gb-285+euro"},
                          name);
    }

    if constexpr(id_ == details::id::IBM01147) {
        return do_compare({"IBM01147", "CCSID01147", "CP01147", "csIBM01147", "ebcdic-fr-297+euro"},
                          name);
    }

    if constexpr(id_ == details::id::IBM01148) {
        return do_compare(
            {"IBM01148", "CCSID01148", "CP01148", "csIBM01148", "ebcdic-international-500+euro"},
            name);
    }

    if constexpr(id_ == details::id::IBM01149) {
        return do_compare({"IBM01149", "CCSID01149", "CP01149", "csIBM01149", "ebcdic-is-871+euro"},
                          name);
    }
